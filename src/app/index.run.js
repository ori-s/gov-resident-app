(function () {
    ...
    /** @ngInject */
    function runBlock($rootScope, $timeout, $state, authorization_service, blockUIConfig, $templateCache,ENV) {
        ...
        $rootScope.$on("$stateChangeStart", function (event, toState, toParams, fromState, fromParams) {
            ...
            if (toState.name.indexOf("app.auth_") != 0) {
                if (!authorization_service.isAuthenticated()) {
                    ...
                    if (!toState.isOrganization && !authorization_service.organization) {
                        $state.transitionTo("app.orgs");
                        event.preventDefault();
                    }
                    return;
